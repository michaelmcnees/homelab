...
The 2026-05-22 local prep audit reported 69 declared reservations, 69 tracked reservations, 0 missing entries, and 0 stale entries. `pxe-pikachu` was briefly added during the live McLan audit, then removed after the host was shut down and retired.
...
| Old K3s and PXE entries | `k3s_*`, `pxe_bulbasaur`, `pxe_squirtle`, `px_mew` | Remove after confirming no temporary external routes or PXE workflows still depend on them. `pxe_pikachu` is retired and should not be re-added. |
